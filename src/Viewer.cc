...
void updateUserState(const nite::UserData &user, uint64_t ts) {
  ...
  if(g_skeletonStates[user.getId()] != user.getSkeleton().getState()) {
    switch(g_skeletonStates[user.getId()] = user.getSkeleton().getState()) {
      ...
      case nite::SKELETON_CALIBRATION_ERROR_TORSO: USER_MESSAGE(
        "Calibration Failed... :-|")
        break;
    }
  }
...
void
DrawStatusLabel(nite::UserTracker *pUserTracker, const nite::UserData &user) {
  int color = user.getId() % colorCount;
  glColor3f(1.0f - Colors[color][0], 1.0f - Colors[color][1],
            1.0f - Colors[color][2]);

  float x, y;
  pUserTracker->convertJointCoordinatesToDepth(user.getCenterOfMass().x,
                                               user.getCenterOfMass().y,
                                               user.getCenterOfMass().z, &x,
                                               &y);
...
void
DrawCenterOfMass(nite::UserTracker *pUserTracker, const nite::UserData &user) {
...
void DrawLimb(nite::UserTracker *pUserTracker,
              ...
              const nite::SkeletonJoint &joint2, int color) {
  float coordinates[6] = {0};
  pUserTracker->convertJointCoordinatesToDepth(joint1.getPosition().x,
                                               joint1.getPosition().y,
                                               joint1.getPosition().z,
                                               &coordinates[0],
                                               &coordinates[1]);

  pUserTracker->convertJointCoordinatesToDepth(joint2.getPosition().x,
  ...
  if(joint1.getPositionConfidence() == 1 &&
     joint2.getPositionConfidence() == 1) {
    glColor3f(1.0f - Colors[color][0], 1.0f - Colors[color][1],
              1.0f - Colors[color][2]);
  }
...
void
DrawSkeleton(nite::UserTracker *pUserTracker, const nite::UserData &userData) {
  DrawLimb(pUserTracker, userData.getSkeleton().getJoint(nite::JOINT_HEAD),
           userData.getSkeleton()
             .getJoint(nite::JOINT_NECK),
           userData.getId() % colorCount);

  DrawLimb(pUserTracker,
           userData.getSkeleton().getJoint(nite::JOINT_LEFT_SHOULDER),
           userData.getSkeleton()
             .getJoint(nite::JOINT_LEFT_ELBOW),
           userData.getId() % colorCount);
  DrawLimb(pUserTracker,
...
void SampleViewer::Display() {
  ...
  if(m_pTexMap == NULL) {
    // Texture map init
    m_nTexMapX = MIN_CHUNKS_SIZE(depthFrame.getVideoMode().getResolutionX(),
                                 TEXTURE_SIZE);
    m_nTexMapY = MIN_CHUNKS_SIZE(depthFrame.getVideoMode().getResolutionY(),
                                 TEXTURE_SIZE);
  ...
  if(depthFrame.isValid() && g_drawDepth) {
    ...
    openni::RGB888Pixel *pTexRow =
      m_pTexMap + depthFrame.getCropOriginY() * m_nTexMapX;
    ...
    for(int y = 0; y < depthFrame.getHeight(); ++y) {
      ...
      for(int x = 0;
          x < depthFrame.getWidth(); ++x, ++pDepth, ++pTex, ++pLabels) {
        if(*pDepth != 0) {
          if(*pLabels == 0) {
            if(!g_drawBackground) {
...
// 					}
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, m_nTexMapX, m_nTexMapY, 0, GL_RGB,
               GL_UNSIGNED_BYTE, m_pTexMap);
  ...
  // bottom right
  glTexCoord2f((float) g_nXRes / (float) m_nTexMapX,
               (float) g_nYRes / (float) m_nTexMapY);
  ...
  //check user 1 and get hand image from him here
  ...
  if (users.getSize() > 0) {
      if(users[0].getSkeleton().getState() == nite::SKELETON_TRACKED) {
        ...
        NitePoint3f leftSholderNite = users[0].getSkeleton()
          ...
          .getPosition();
        NitePoint3f rightSholderNite  = users[0].getSkeleton()
          .getJoint(nite::JOINT_RIGHT_SHOULDER)
          .getPosition();
        NitePoint3f rightHipNite = users[0].getSkeleton()
          .getJoint(nite::JOINT_RIGHT_HIP)
          .getPosition();
        NitePoint3f rightHandNite = users[0].getSkeleton()
          .getJoint(nite::JOINT_RIGHT_HAND)
          .getPosition();
        ...
        glm::vec3 sphericalCoords{glm::length(rightArm), alpha, phi};
      };
  }
  ...
  for(int i = 0; i < users.getSize(); ++i) {
    ...
    if(user.isNew()) {
      ...
      m_pUserTracker->startPoseDetection(user.getId(),
                                         nite::POSE_CROSSED_HANDS);
    }
    else if(!user.isLost()) {
    ...
    if(m_poseUser == 0 || m_poseUser == user.getId()) {
      ...
      if(pose.isEntered()) {
        ...
        // tick
        if(userTrackerFrame.getTimestamp() - m_poseTime >
           g_poseTimeoutToExit * 1000) {
